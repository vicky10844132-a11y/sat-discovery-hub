...
style.textContent = `
  ...
  .spaceopsSharedGlobeHost{position:absolute!important;inset:0!important;z-index:2!important;overflow:hidden!important;background:transparent!important}
  ...
  .spaceopsGlobeActive>.map{background:
...
host.setAttribute('aria-label', 'Shared interactive 3D Earth');
...
const world = new Globe(host, {
  ...
  .pointAltitude('alt')
  .pointColor('color')
  .pointRadius(d => d.id === shared.selectedId ? 0.32 : d.kind === 'ship' ? 0.16 : 0.22)
  ...
  .polygonCapColor(() => 'rgba(223,63,118,.16)')
  .polygonSideColor(() => 'rgba(223,63,118,.04)')
  .polygonStrokeColor(() => '#ff6d9f')
  ...
  .customThreeObject(d => {
    ...
    const glow = new THREE.Mesh(
      new THREE.SphereGeometry(2.9,12,12),
  ...
  .customThreeObjectUpdate((obj,d) => {
    ...
    obj.scale.setScalar(selected ? 1.45 : 1.0);
  });
...
function currentProfile() {
  ...
  const chips = [...document.querySelectorAll('.sceneTools .chip,.maptools .chip')];
  ...
  profile.links = read(['LINK'],profile.links);
  return profile;
}
...
function refreshStaticLayers(force=false) {
...
function resize() {
  const w = Math.max(1,scene.clientWidth);
...
document.addEventListener('click',e => {
  const el = e.target.closest?.('.sceneTools .chip,.maptools .chip,.zoomtools button,.reset');
  if (!el) return;
  setTimeout(() => {
...
let raf = 0;
function tick(now) {
  const elapsed = (Date.now() - shared.epochMs) / 1000;
  sats.forEach(def => {
    const angle = def.phase + elapsed * (Math.PI*2/def.visualPeriod);
    const p = orbitPoint(def,angle);
    def.lat = p.lat;
    def.lng = p.lng;
    def.alt = p.alt;
  });
  const profile = refreshStaticLayers();
  if (profile.sats) world.customLayerData(world.customLayerData());
  updateArc(profile,now);
